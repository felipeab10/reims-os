...
class T007HostRebootTests(unittest.TestCase):
    ...
    def test_supervisor_reboot_integration(self):
        p,s=self.run_supervisor("restart-exit","installed"); result=json.loads((s/"result.json").read_text()); audit=json.loads((s/"host-action.json").read_text()); self.assertEqual(p.returncode,0); self.assertEqual(result["classification"],"GUEST_REBOOT"); self.assertEqual(result["classification_reason"],"qmp_guest_reset_shutdown"); self.assertEqual(audit["status"],"WOULD_REBOOT"); self.assertEqual(audit["action"],"reboot"); events=[json.loads(x)["event"] for x in (s/"lifecycle.log").read_text().splitlines()]; self.assertLess(events.index("classification"),events.index("WOULD_REBOOT")); print("T007_SUPERVISOR_INTEGRATION=PASS"); print("T007_QEMU_REBOOT_SHUTDOWN_CAUSE=PASS"); print("T007_CLASSIFICATION_BEFORE_ACTION=PASS")
