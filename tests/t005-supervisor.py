#!/usr/bin/env python3
import json, re, signal, subprocess, sys, tempfile, time, unittest
...
class SupervisorTests(unittest.TestCase):
    def setUp(self):
        self.tmp=Path(tempfile.mkdtemp()); self.run=self.tmp/'run'; self.logs=self.tmp/'logs'
    ...
    def launch(self, mode='shutdown', state='installed', rc=0, hold=.15, delay=0, script=None):
        ...
        p=subprocess.run([sys.executable,str(SUP),'run','--vm-id','reims-0123456789abcdef','--appliance-state',state,'--run-dir',str(self.run),'--log-root',str(self.logs),'--']+self.command(script,mode,state,rc,hold,delay),cwd=ROOT)
    ...
    def test_install_reset_temporal(self):
        proc=subprocess.Popen([sys.executable,str(SUP),'run','--vm-id','reims-x','--appliance-state','installing','--run-dir',str(self.run),'--log-root',str(self.logs),'--']+self.command(self.fake,'install-reset','installing',0,.1,0),cwd=ROOT)
    ...
    def test_pid_identity_and_ambiguity(self):
        ...
        cmd=[sys.executable,str(SUP),'run','--vm-id','reims-x','--appliance-state','installed','--run-dir',str(self.run),'--log-root',str(self.logs),'--',str(amb),str(self.run)]; p=subprocess.Popen(cmd,cwd=ROOT); _,r=self.result_for(p); self.assertIsNone(r['qemu_pid']); self.assertIn('qemu_pid_ambiguous',r['limitations']); print('T005_QEMU_PID_AMBIGUITY=PASS')
