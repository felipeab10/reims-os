...
try:
 ...
 if mode == 'invalid': c.sendall(b'{"bad":true}\n')
  ...
  elif mode in ('shutdown','terminal','serial-ambiguous'): c.sendall(b'{"event":"SHUTDOWN","data":{"guest":true,"reason":"guest-shutdown"}}\n')
  elif mode in ('reset','late-panic','install-reset','ambiguity'): c.sendall(b'{"event":"RESET","data":{"guest":true,"reason":"guest-reset"}}\n')
  elif mode == 'restart-exit': c.sendall(b'{"event":"SHUTDOWN","data":{"guest":true,"reason":"guest-reset"}}\n')
